...
def precessingTrain(trainDataPath, T='a', length=784, top_num=60000):
    """
    预处理训练数据
    ...
    for week in os.listdir(trainDataPath):
        ...
        for day in os.listdir(week_path):
            ...
            format_data_path = os.path.join(day_path, length)
            tcpdmp = "{}/tcpdump".format(day_path)
            label_file = "{}/tcpdump.list".format(day_path)
            pcap2session_cmd = "bash pcap2session.sh {} {} -f".format(tcpdmp, os.path.join(day_path, flow_path))
            print(pcap2session_cmd)
            processSeeion_cmd = "bash processSeeion.sh {}/{}/ {}/ {} {}".format(flow_path, session_type, format_data_path, top_num, length)
            print(processSeeion_cmd)
